...
class MainWindow(Tk):
    def __init__(self):
        ...
        Label(self, text="Введіть розмір вибірки (за одну секунду):").pack()
        ...
        Label(self, text="Введіть кількість процесів (стандартно кількість ядер*2):").pack()
        self.p_input = Entry(self, width=35)
        self.p_input.pack()
        Label(self, text="Введіть кількість переобчислень:").pack()
        ...
        self.signal_chooser = tkinter.ttk.Combobox(state="readonly", values=["синус", "косинус", "східчастий",
                                                                             "пилоподібний", "трикутний", "прямокутний імпульс"])
        self.signal_chooser.current(0)
        self.signal_chooser.pack()
        Label(self, text="Введіть лінійну частоту сигналу (Гц):").pack()
        ...
        Label(self, text="Введіть стандартне відхилення / кількість імпульсних завад:").pack()
        self.std_dev = Entry(self, width=35)
        self.std_dev.pack(pady=5)
        Button(self, text="Виконати фільтрацію сигналу", command=self.solve_problem).pack(pady=20)
        Button(self, text="Показати графічне представлення результатів", command=self.show_results).pack()
    ...
    def solve_problem(self):
        ...
        self.label_mse.config(text=f'Середньоквадратична похибка: {mse}')
        self.label_mse.pack(pady=5)
        res_equal_resp = "Так" if is_res_equal else "Ні"
        self.label_equal.config(text=f'Результати послідовного і паралельного виконання співпадають?\n {res_equal_resp}')
