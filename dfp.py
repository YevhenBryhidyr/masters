...
def show_results(clean_signal, noisy_signal, x_seq, x_par, t, window_size=1000):
    N = len(t)

    if N > window_size:
        start_idx = (N // 2) - (window_size // 2)
    ...
    clean_slice = clean_signal[start_idx:end_idx]
    noisy_slice = noisy_signal[start_idx:end_idx]
    x_seq_slice = x_seq[start_idx:end_idx]
    x_par_slice = x_par[start_idx:end_idx]

    plt.figure(figsize=(10, 4))
    plt.plot(t_slice, noisy_slice, label='Сигнал з шумом')  # alpha робить шум прозорішим
    plt.plot(t_slice, clean_slice, label='Чистий сигнал', linewidth=2)
    plt.plot(t_slice, x_seq_slice, label='Відфільтрований сигнал', color='red', linewidth=2)
    ...
    plt.legend()
    plt.xlabel('Час [с]', fontsize=fontsize)
    plt.ylabel('Значення сигналу', fontsize=fontsize)
    plt.grid(True)
    plt.tight_layout()

    plt.figure()
    plt.plot(t_slice, x_par_slice, label='Відфільтрований сигнал (паралельне виконання)', color='blue', linewidth=1)
...
def find_mse_single(x, clean_signal):
    mse = np.mean((x - clean_signal) ** 2)
    return mse
